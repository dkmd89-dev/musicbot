...
| Datei | Status | Kurzthema |
|---|---|---|
| [MusicBot_ENGINEERING_BASELINE_v10.md](MusicBot_ENGINEERING_BASELINE_v10.md) | DRAFT / IN PROGRESS (nicht eingefroren) — laufender Zwischenstand seit dem v9-Freeze (ARCH Status / Recent Major Changes / Testzahlen); Findings weiterhin in [FINDINGS_INDEX.md](FINDINGS_INDEX.md) | Erster Eintrag: Artist-Identity-Resolution-Migration (Phase A–F, PR #176). Aktuellster Eintrag: ARCH-027 — Error Handler Consolidation (Single Shared Instance, `bot.py`+`rich_menu_handler.py`), noch nicht gemergt. Vollsuite (Nutzer, Stand ARCH-025 Closure/PR #207): 3460 passed / 1 skipped / 11 subtests passed / 0 failed — ARCH-026/027 ohne neuen Full-Suite-Lauf. |
| [MusicBot_ENGINEERING_BASELINE_v9.md](MusicBot_ENGINEERING_BASELINE_v9.md) | BASELINE (eingefrorener Referenzpunkt nach Freeze 2026-09-07 — für den laufenden Stand siehe [MusicBot_ENGINEERING_BASELINE_v10.md](MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT) + [FINDINGS_INDEX.md](FINDINGS_INDEX.md)) | 61 PRs seit v8 (#104–#164): Findings-/Cleanup-Vorlauf (Security-Fix BOT_TOKEN-Leak, YTPARSE-01, Genre-System/ARCH-022, Telegram-Erweiterungen), Phase 1 „Library Health Scanner" (PR #145–#147), Phase 2 „Smart Library Repair" (PR #146/#148–#156, Loudness-Produktionslauf 133/133 SUCCESS), Phase 3 „Music Quality & Library Intelligence" (PR #157–#164). 10 von 11 in v8 gelisteten akzeptierten Risiken geschlossen, nur INV-01 bleibt offen. 2580 passed / 0 failed (1 umgebungsbedingt skipped) — Änderungen seither → v10 (DRAFT) |
...
| [MusicBot_ARCH-027_Error_Handler_Consolidation.md](MusicBot_ARCH-027_Error_Handler_Consolidation.md) | LIVING | **IMPLEMENTATION COMPLETE** (statisch verifiziert, noch nicht gemergt). Setzt den ARCH-026-Fix-Plan für F1-F3 um: `RichMenuHandler.__init__()` erhält optionalen Parameter `error_handler=None`, `bot.py` injiziert seine PTB-registrierte Instanz per Constructor Injection statt `RichMenuHandler.initialize()` eine zweite erzeugen zu lassen (kontrollierter Fallback bleibt für Standalone-/Test-Konstruktion). Nur 2 Dateien geändert (`bot.py`, `handlers/menu/rich_menu_handler.py`), additiv, alle bestehenden Testkonstruktionsstellen unverändert kompatibel. Ursache der leeren Admin-Berichte konkret bewiesen (nicht nur vermutet): einzige `ErrorHandlerAdminInterface` beobachtete die falsche von zwei Instanzen. F1/F2/F3 CLOSED, F4-F12 unverändert außerhalb Scope. Modularisierungs-Assessment für `enhanced_error_handler.py`: NOT WARRANTED. |
