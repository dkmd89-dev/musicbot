...
| [MusicBot_ARCH-031_Library_Repair_Telegram_Integration_Characterization.md](MusicBot_ARCH-031_Library_Repair_Telegram_Integration_Characterization.md) | LIVING (Characterization + verbindliche Architekturentscheidung abgeschlossen 2026-09-14, Umsetzung ausstehend) | **Verbindliche** Zielarchitektur + Datei→Funktion→Zielmodul→Tests-Migrationsplan für vollständige Telegram-Ausführbarkeit von Executor Level 1–3 und Konsolidierung der drei neuen Wartungsskripte (`fix_artist_casing.py`/`remove_legacy_genre_atom.py`/`set_genre.py`) in `services/library_repair/`. Löst den ursprünglichen Auftrags-Bezeichner "ARCH-030" wegen Nummernkollision (bereits vergeben, siehe Zeile unten) auf ARCH-031 auf; Folge-Implementierungsphasen ARCH-032 (Maintenance Consolidation) / ARCH-033 (Telegram Level-2/3 Repair). Siehe `adr/0001`–`0004` und C4-/Sequenzdiagramme unter `diagrams/`. Keine Code-Änderung in dieser Phase. Vorphasen-Entwurf (Ausgangspunkt, jetzt SUPERSEDED): [designs/library-repair-telegram-integration-overview.md](designs/library-repair-telegram-integration-overview.md). |

## Baseline

| Datei | Status | Kurzthema |
|---|---|---|
| [MusicBot_ENGINEERING_BASELINE_v10.md](MusicBot_ENGINEERING_BASELINE_v10.md) | DRAFT / IN PROGRESS (nicht eingefroren) — laufender Zwischenstand seit dem v9-Freeze (ARCH Status / Recent Major Changes / Testzahlen); Findings weiterhin in [FINDINGS_INDEX.md](FINDINGS_INDEX.md) | Erster Eintrag: Artist-Identity-Resolution-Migration (Phase A–F, PR #176). Aktuellster Eintrag: ARCH-028 — Error Handler Closure (F4/F5/F6 + FamilyChallengeScheduler-Fix), noch nicht gemergt. Vollsuite (Nutzer, Stand ARCH-028): 3490 passed / 1 skipped / 11 subtests passed / 0 failed. |
