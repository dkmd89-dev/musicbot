...
| AUTOLEARN-001 | Bei ENCAP-001 gefunden: `download_utils.py` ruft nach jedem erfolgreichen `EnhancedMetadataProcessor.process_single_track()`-Aufruf zusätzlich `auto_learn_manager.learn_artist(...)` auf (zwei Call-Sites, Playlist- und Single-Track-Pfad) — obwohl `process_single_track()` selbst bereits intern denselben `learn_artist()`-Aufruf für jeden Track macht (Schritt "19b. Auto-Learning"). Die beiden Aufrufe unterscheiden sich in Details: unterschiedliche `_LEARN_SOURCES`-Mengen (intern zusätzlich `"first_artist_from_title"`), unterschiedliche `raw_name`-Herkunft, und der externe Aufruf hat eine zusätzliche `raw.lower() != canonical.lower()`-Bedingung, die der interne nicht hat | P2 | **nicht behoben, nur charakterisiert** — kein Bug im Sinne von falschem Ergebnis: `learn_artist()` ist durch die interne `_is_artist_known()`-Prüfung idempotent/selbstsichernd, ein zweiter Aufruf für denselben Artist schreibt nichts erneut. Trotzdem unklar, ob die zwei leicht unterschiedlichen Bedingungssätze beabsichtigt sind (z.B. der externe Aufruf lernt evtl. Fälle, die der interne aus anderen Gründen verpasst) oder ob es sich um versehentlich redundant gewordenen Code aus einer früheren Iteration handelt — verdient eine eigene, dedizierte Untersuchung mit Vorher/Nachher-Beispielen (Regel 28), nicht einen mechanischen Kapselungs-Fix im Vorbeigehen |
...
- [x] AUTOLEARN-001 charakterisiert, nicht behoben — doppelter `learn_artist()`-Aufruf (intern + `download_utils.py`) mit leicht abweichenden Bedingungen, idempotent/harmlos aber unklar ob beabsichtigt, für dedizierte Folge-Untersuchung vorgemerkt
