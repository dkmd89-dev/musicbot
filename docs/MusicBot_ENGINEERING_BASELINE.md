...
- [ ] Rest der Ungetestet-Liste (podcast_rss_manager.py, file_utils.py — alle live, 2 Dateien; die beiden reinen ABC/Protocol-`interfaces.py`-Dateien haben keine Implementierung und wurden bewusst übersprungen) noch nicht einzeln geprüft. `advanced_podcast_finder.py` bewusst nicht angefasst (eigenständiges manuelles CLI-Tool, kein Bot-Aufrufer, aber unklar ob vom Nutzer aktiv verwendet — nicht ohne Rückfrage löschen)
