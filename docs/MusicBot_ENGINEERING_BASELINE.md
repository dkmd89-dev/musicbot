...
- [ ] Rest der Ungetestet-Liste (services/downloader/download/*.py, progress_tracker.py, organizer.py, playlist_processor.py u. a. — alle live, ~15 Dateien) noch nicht einzeln geprüft. `advanced_podcast_finder.py` bewusst nicht angefasst (eigenständiges manuelles CLI-Tool, kein Bot-Aufrufer, aber unklar ob vom Nutzer aktiv verwendet — nicht ohne Rückfrage löschen)
