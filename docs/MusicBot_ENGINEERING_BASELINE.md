...
- [ ] Zielarchitektur schrittweise weiter umsetzen (die drei zurückgestellten DEFER-Punkte aus ARCH-004 Abschnitt 7, die in ARCH-005 Abschnitt 5 dokumentierten Restrisiken, und die `handlers.duplicate_handler.DuplicateEntry`-Abhängigkeit aus ARCH-007 Abschnitt 4 — je eigene Nutzerentscheidung nötig). Neuer, vom Post-ARCH-009-Audit empfohlener nächster Kandidat (P-1): `handlers/admin/bot_restart_handler.py::_trigger_restart()` nach `utils/` extrahieren — siehe `docs/MusicBot_POST-ARCH-009_Audit.md`, Abschnitt 5
