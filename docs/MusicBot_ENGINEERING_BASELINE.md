...
| ARTISTNORM-001 | Bei AUTOLEARN-001 gefunden: `ArtistNormalizer.normalize()` mangelt Namen, die die Zeichenfolge "ft" enthalten, auch wenn sie kein Featuring-Trennzeichen sind — konkretes Beispiel: `normalize("Hardenacke trifft")` liefert `"Hardenacke Trif"` (das deutsche Wort „trifft" wird zu „Trif" verstümmelt). Vermutete Ursache: die Collaboration-/Featuring-Split-Logik (`split_main_and_featuring` bzw. eine ähnliche Regex in `utils/artist_map.py`) matcht vermutlich ein einfaches `"ft"`-Substring-Pattern statt eines wortgrenzen-bewussten `"\bft\b"`/`"feat"`-Patterns. Betrifft real existierende Kanalnamen (`"Hardenacke trifft"` steht in `mapping/special_channel.yaml`) | P2 | **nicht behoben, nur charakterisiert** — reproduziert via direktem `ArtistNormalizer.normalize()`-Aufruf. Nicht im Rahmen von AUTOLEARN-001 gefixt, da eine Änderung an der Collaboration-Split-Regex einen viel größeren Blast-Radius hat (betrifft jede Artist-Normalisierung im Projekt, nicht nur Sonderkanal-Erkennung) und eigene Vorher/Nachher-Beispiele + Tests braucht (Regel 28), bevor daran etwas geändert wird |
...
- [x] ARTISTNORM-001 charakterisiert, nicht behoben — `ArtistNormalizer.normalize()` mangelt Namen mit "ft"-Substring (z. B. "Hardenacke trifft" → "Hardenacke Trif"), größerer Blast-Radius, eigene Untersuchung nötig
