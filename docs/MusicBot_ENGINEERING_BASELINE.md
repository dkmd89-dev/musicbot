...
- [ ] Rest der Ungetestet-Liste (file_utils.py — 1 Datei, live) noch nicht geprüft. Die beiden reinen ABC/Protocol-`interfaces.py`-Dateien haben keine Implementierung und wurden bewusst übersprungen. `advanced_podcast_finder.py` bewusst nicht angefasst (eigenständiges manuelles CLI-Tool, kein Bot-Aufrufer, aber unklar ob vom Nutzer aktiv verwendet — nicht ohne Rückfrage löschen)
