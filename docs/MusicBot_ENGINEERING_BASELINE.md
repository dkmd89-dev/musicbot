...
| DATA-001 | Beim Migrieren der GENRE-002-Artist-Einträge zufällig gefunden: `mapping/artist_genre.yaml` hat 12 doppelte Top-Level-Keys (`dominic fike`, `eminem`, `herzchen`, `majan`, `dasha`, `sarah engels`, `taylor swift`, `calvin harris`, `riton`, `one-t`, `fayan`, `"The Weeknd"`) — z.B. Zeile 69 und 443 (`dominic fike`), Zeile 888 und 924 (`"The Weeknd"`, mit Kommentar „Korrektur für The Weeknd (falls Fuzzy-Match falsch war)" bei der zweiten). PyYAML behält beim Laden nur den JEWEILS LETZTEN Wert pro Key — die erste Definition wird still verworfen, ohne Fehler oder Warnung | P1 | **offen, nicht Teil dieser Änderung** — reine Mapping-Datenqualität, nicht Code; braucht Sichtung Zeile für Zeile (welcher der beiden Werte ist der gewollte/aktuellere?), keine mechanische Auflösung ohne Entscheidung, welche Version jeweils stimmt (analog GENRE-002/Regel 3, kein Bulk-Fix) |
