...
| DOC-001 | README dokumentierte System kaum (2 Zeilen: Projektname + eine Satz-Beschreibung) | P1 | **behoben** — README neu geschrieben: Was der Bot macht, vereinfachte Architektur, Projektstruktur-Tabelle, Setup (`.env`-Variablen), Start-/Testbefehle, Hinweis auf Mapping-Dateien als Fachlogik, Verweis auf `CLAUDE.md`/Engineering-Baseline für Details. Dabei aufgefallen (dokumentiert, nicht behoben): keine `requirements.txt` im Repository — Python-Abhängigkeiten sind nur implizit über Imports im Code bestimmbar |
...
- [ ] Telegram-Handler-Layer testen — begonnen mit `UserManagementHandler` (TEST-009/SEC-005), `BackupHandler` (TEST-010/SEC-006), `rich_menu_system.py` (TEST-011), `enhanced_error_handler.py` (BUG-005), `rich_menu_handler.py` (BUG-006) und `navidrome_menu_handler.py` (BUG-007); `enhanced_status_handler.py`, `mugge_statistik_handler.py` weiterhin ungetestet (~1.400 Zeilen)
