...
- [ ] Rest der Ungetestet-Liste (services/downloader/download/*.py, progress_tracker.py, organizer.py, playlist_processor.py, handlers/test_menu_handler.py u. a. — alle live, ~13 Dateien; die beiden reinen ABC/Protocol-`interfaces.py`-Dateien haben keine Implementierung und wurden bewusst übersprungen) noch nicht einzeln geprüft. `advanced_podcast_finder.py` bewusst nicht angefasst (eigenständiges manuelles CLI-Tool, kein Bot-Aufrufer, aber unklar ob vom Nutzer aktiv verwendet — nicht ohne Rückfrage löschen)
