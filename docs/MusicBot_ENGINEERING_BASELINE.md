...
| BUG-001 | `MusicBrainzClient._build_metadata()` setzt `"track_number"` auf `first_release.get("medium-track-count")`. Laut musicbrainzngs-Quellcode (`mbxml.py`) ist das die GESAMTANZAHL der Tracks auf dem Medium (`ws2:track-count`), nicht die Position des gefundenen Recordings — jeder Track desselben Albums bekäme denselben falschen Wert (z.B. immer „12“ bei einem 12-Track-Album) | P2 | **nicht gefixt, bewusst nur charakterisiert** — vollständiger Repo-weiter Grep bestätigt: kein einziger Aufrufer liest `mb_data["track_number"]` aus `fetch_metadata()` (die echte `track_number` in `DownloadResult`/`MetadataResult` stammt aus `album_processor.extract_track_number_from_string()`, einer anderen Quelle). Aktuell folgenlos, da toter Wert — ein echter Fix bräuchte Zugriff auf die tatsächliche Track-Position innerhalb der Medium-Tracklist, die im aktuellen Datenfluss nach dem Recording-Matching nicht mehr vorgehalten wird (ähnlich GENRE-002: kein mechanischer Fix, sondern ein kleiner Design-Schritt). Regressionstest, der das aktuelle (falsche) Verhalten einfriert: `tests/test_musicbrainz_client.py::TestBuildMetadataFieldExtraction::test_track_number_is_actually_the_medium_total_track_count_not_position` |
...
- [x] TEST-006 behoben — 25 Characterization-Tests für `MusicBrainzClient` (P0, vorher 0 Tests), Nebenbefund BUG-001 (`track_number`-Feld inhaltlich falsch, aber tot) dokumentiert
