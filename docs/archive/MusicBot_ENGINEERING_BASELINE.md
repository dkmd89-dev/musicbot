...
| TEST-013 | `utils/youtube_parser.py` (546 Zeilen, P0-Pfad — eingebunden in `enhanced_metadata_processor.py`, `download_utils.py`, `playlist_processor.py`, `duplicate_handler.py`) hatte 0 dedizierte Tests, gefunden über eine systematische Ungetestet-Prüfung aller Quelldateien unter `services/`, `handlers/`, `klassen/`, `utils/`, `api/` gegen `tests/`-Referenzen | P1 | **behoben** — 43 neue Characterization-Tests in `tests/test_youtube_parser.py`. 7 davon 1:1 aus dem in der Produktionsdatei eingebauten Selbsttest-Block (`if __name__ == "__main__":`) übernommen, vor dem Schreiben per `python3 utils/youtube_parser.py` verifiziert, dass alle 7 tatsächlich bestehen (dokumentiertes Soll-Verhalten, nicht geraten). Zusätzlich Edge-Cases (leerer/`None`-Titel, kein Trennzeichen, `by`-Separator) und alle internen Helferfunktionen einzeln (`_split_multi_artists`, `_extract_features`, `_parse_artist_and_title`, `_clean_bracket_content`, `_clean_title_suffixes`, `_normalize_string`, `quick_parse`). Ein eigener Test-Block (`TestFeatFtSubstringSafety`) verankert dauerhaft das bei ARTISTNORM-002 nachgewiesene sichere Verhalten der vier feat/ft-Patterns in dieser Datei (zwingendes `\s+` statt `\s*`) als Regressionsschutz. Kein neuer Bug gefunden — reine Charakterisierung, keine Produktionscode-Änderung. Voller Regressionslauf: 661 bestanden (vorher 618), unverändert 15 Vorbestand-Fehler |
| LEGACY-008 | Bei der Fortsetzung der systematischen Ungetestet-Prüfung gefunden: `handlers/message_handler.py` (9 Funktionen, u. a. `handle_message`/`process_audio_download`) und `handlers/telegram_handler.py` (nur ein 4-Zeilen-Kommentarstub ohne jeden ausführbaren Code) — beide null Aufrufer im gesamten Repo, unverändert seit dem Initial-Commit, derselbe abgelöste Vor-`RichMenuSystem`-Cluster wie LEGACY-005/007 | P2 | **entfernt** — beide Dateien gelöscht, frisch verifiziert (keine Aufrufer, keine Testreferenzen). Voller Regressionslauf: unverändert 15 Vorbestand-Fehler, 661 bestanden |
| TEST-014 | `services/downloader/utils/metadata/lyrics_processor.py` (`LyricsProcessor`, P0-Metadaten-Sub-Prozessor, eingebunden in `EnhancedMetadataProcessor`) hatte 0 Tests, gefunden über dieselbe systematische Ungetestet-Prüfung wie TEST-013 | P1 | **behoben** — 13 neue Tests in `tests/test_lyrics_processor.py` (`genius_client` gemockt, Regel 7): `fetch_lyrics()` Erfolg/kein Ergebnis/Exception, `fetch_lyrics_with_fallback()`s vollständige Fallback-Kette (Haupt-Artist-Erfolg überspringt Fallback, Fallback bei Miss, mehrere Fallback-Artists in Reihenfolge, `None`/leere Fallback-Liste, Identitäts-Check überspringt Fallback-Namen die dem Hauptartist entsprechen). Dabei ein Detail charakterisiert (kein Bug): ein Fallback-Eintrag wie `"  "` (nur Leerzeichen) ist selbst truthy und wird NICHT durch die Identitäts-Prüfung übersprungen (die vergleicht erst nach `.strip()`), sondern tatsächlich als Anfrage versucht — harmlos, da der Genius-Client dafür einfach kein Ergebnis liefert. Kein neuer Bug gefunden, reine Charakterisierung. Voller Regressionslauf: 674 bestanden (vorher 661), unverändert 15 Vorbestand-Fehler |
| BUG-008 | Beim Fortsetzen der systematischen Ungetestet-Prüfung gefunden: der „Navidrome Scan"-Button im Admin-Menü (`RichMenuHandler._handle_navidrome_scan()`, verdrahtet über `_register_system_handlers()`/`add_child_menu_item()`, siehe ENCAP-001) war end-to-end unerreichbar. Er rief `self.navidrome_adapter.trigger_scan()` auf, aber `NavidromeAdapter` (`handlers/adapters/navidrome_adapter.py`) wurde **nirgends im Repo instanziiert** — `self.navidrome_adapter` blieb strukturell für immer `None`, jeder Klick zeigte nur „Navidrome-Adapter nicht verfügbar". Zusätzlich lag eine zweite, unabhängige, bereits getestete Implementierung (`api/navidrome_api.py::NavidromeAPI.execute_scan()`, TEST-005) komplett brach — ebenfalls nirgends aufgerufen. `NavidromeAdapter.trigger_scan()` nutzte zudem blockierendes `subprocess.run()` (derselbe Bug-Typ wie REL-004 aus einer früheren Session), während `execute_scan()` bereits async (`subprocess_shell`) mit Timeout-Handling implementiert war | P1 | **behoben, nach expliziter Nutzer-Entscheidung** (Regel 27 — keine parallele Logik pflegen, wenn vorhandene erweitert werden kann): `_handle_navidrome_scan()` ruft jetzt direkt `NavidromeAPI.execute_scan()` auf (async, bereits getestet, liefert fertig MarkdownV2-formatierte Nachricht — `query.edit_message_text(..., parse_mode="MarkdownV2")`). Die nie funktionsfähige `NavidromeAdapter`-Klasse als überholt entfernt (`handlers/adapters/navidrome_adapter.py`), inkl. der toten `self.navidrome_adapter`-Attribut-/`set_navidrome_adapter()`-Verdrahtung in `RichMenuHandler`. 4 neue Regressionstests in `tests/test_rich_menu_handler.py::TestHandleNavidromeScan` (Erfolg, Fehlschlag-Nachricht, Admin-Gate weiterhin durchgesetzt, Exception-Handling), per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert. Voller Regressionslauf: 678 bestanden (vorher 674), unverändert 15 Vorbestand-Fehler |
| LEGACY-009 | Bei derselben Untersuchung gefunden: `utils/yt_utils.py` (`get_youtube_thumbnail()`) — 0 echte Aufrufer im gesamten Repo (der Modulname „yt_utils" taucht nur zufällig als interner Logger-Name in `youtube_parser.py` auf, keine echte Import-Beziehung), unverändert seit dem Initial-Commit | P2 | **entfernt** — Datei gelöscht, frisch verifiziert (keine Aufrufer, keine Testreferenzen). Voller Regressionslauf: unverändert 15 Vorbestand-Fehler, 678 bestanden |
| TEST-015 | `handlers/admin/bot_restart_handler.py` (`BotRestartHandler`, live verdrahtet in `RichMenuHandler.initialize()`, administrativ riskant — löst `sudo systemctl restart bot` aus) hatte 0 Tests | P1 | **behoben** — 12 neue Tests in `tests/test_bot_restart_handler.py`, `subprocess.run` in JEDEM Test gemockt (Regel 7 — niemals einen echten Neustart auslösen). Dabei eine potenzielle Sicherheitsfrage geprüft und als sicher bestätigt (kein Bug): `cancel_restart()` prüft `_is_admin` selbst NICHT, im Gegensatz zu `show_restart_confirm()`/`execute_restart()` — verifiziert, dass `_handle_restart_callback()` (`rich_menu_system.py`) der einzige Aufrufpfad für alle drei `restart:*`-Callbacks ist und bereits VOR dem Dispatch eine eigene `_is_admin_check()`-Prüfung macht (`_ADMIN_ONLY_PREFIXES` deckt `restart:` zwar nicht ab, aber der Dispatcher hat eine eigene, wirksame Prüfung). `cancel_restart()` kann daher nie ohne vorherige Admin-Prüfung erreicht werden — redundanter Stil, keine Lücke. Kein neuer Bug gefunden. Voller Regressionslauf: 690 bestanden (vorher 678), unverändert 15 Vorbestand-Fehler |
| BUG-009 | `services/downloader/utils/progress_tracker.py` (`ProgressTracker`, live genutzt in `DownloadHandler`/`EnhancedDownloadProcessor`, vorher 0 Tests) — beim Charakterisieren gefunden: `EnhancedDownloadProcessor.cleanup()` (`download_utils.py`, implementiert das `DownloadCoordinator`-Protokoll) ruft `self.tracker.cleanup()` auf, aber `ProgressTracker` hatte **keine `cleanup()`-Methode** — ein `AttributeError`, sobald `self.tracker` ein echter `ProgressTracker` ist. Aktuell unerreichbar: `EnhancedDownloadProcessor.init_tracker()` (der einzige Pfad, der `self.tracker` setzt) wird nirgends im Repo aufgerufen — aber ein echter, sofortiger Crash, falls das je verdrahtet wird | P2 | **behoben** — `ProgressTracker.cleanup()` als No-Op-Methode ergänzt (die Klasse hält keine eigenen freizugebenden Ressourcen, nur Zähler/Zeitstempel/eine fremde `Update`-Referenz). 16 neue Tests in `tests/test_progress_tracker.py`, inkl. eines expliziten Regressionstests, der `EnhancedDownloadProcessor.cleanup()` mit einem echten `ProgressTracker` als `self.tracker` aufruft (via `object.__new__()`, um die schwere `SingletonMixin`-Initialisierung zu umgehen) — per `git stash` gegen den Vor-Fix-Stand als fehlschlagend (`AttributeError`) verifiziert. Nebenbei charakterisiert: `update_progress()`s Drossel-Logik (`update_interval=5s`) sendet bei `total_items > 1` beim ERSTEN Aufruf standardmäßig KEINE Nachricht, solange keine 5 echten Sekunden vergangen sind (nur beim letzten Item wird immer gesendet) — kein Bug, aber nicht offensichtlich aus dem Code ersichtlich. `set_current_item()`, `progress_hook()`, `track_performance()` (Modul-Funktionen) haben aktuell keine Aufrufer im Repo — funktionieren korrekt, bewusst nicht entfernt (kein Hinweis, dass sie dauerhaft ungenutzt bleiben sollen). Voller Regressionslauf: 706 bestanden (vorher 690), unverändert 15 Vorbestand-Fehler |
| LEGACY-010 | Fortsetzung der systematischen Ungetestet-Prüfung: `utils/organizer.py` (`move_file_based_on_metadata()`) — 0 Aufrufer im gesamten Repo, unverändert seit dem Initial-Commit, vermutlich abgelöst durch `FilenameFixerTool.move_to_library()`/`build_final_path()` (die tatsächlich genutzte, u. a. in SEC-002 gehärtete Mechanik). Zusätzlich in `utils/file_ops.py` gefunden: die dortige eigene `safe_rename()`-Implementierung wurde NIE aufgerufen — `utils/helpers.py` hat eine eigene, unabhängige `safe_rename()`, die zwar `IO_SEMAPHORE`/`atomic_rename()` aus `file_ops.py` importiert, aber ihre eigene Retry-Fallback-Logik hat; `move_to_processed()` hatte ebenfalls 0 Aufrufer | P2 | **entfernt** — `utils/organizer.py` komplett gelöscht. In `utils/file_ops.py` nur die beiden toten Funktionen (`safe_rename`, `move_to_processed`) entfernt, `atomic_rename()`/`IO_SEMAPHORE` bewusst behalten (live, indirekt über `utils/helpers.py::safe_rename()` im P0-Pfad `FilenameFixerTool` genutzt). 6 neue Tests in `tests/test_file_ops.py` für die verbliebene, tatsächlich genutzte Funktionalität. Voller Regressionslauf: 712 bestanden (vorher 706), unverändert 15 Vorbestand-Fehler |
| LEGACY-011 | Fortsetzung der systematischen Ungetestet-Prüfung: `services/status_service.py` (globale Status-Zähler-Funktionen) — 0 Aufrufer, unverändert seit dem Initial-Commit, vermutlich abgelöst durch `SystemMonitor`/`BotStatusTracker` in `enhanced_status_handler.py` (TEST-012). `services/organizer.py` (1082 Zeilen, `MusicOrganizer`, `if __name__ == "__main__":` mit `argparse`) — kein Bot-Aufrufer, aber ein eigenständiges CLI-Skript (analog `advanced_podcast_finder.py`), daher explizit beim Nutzer nachgefragt statt automatisch als Legacy eingestuft: Nutzer bestätigte, dass es nicht mehr manuell verwendet wird. `services/notification_service.py` (`NotificationService`) hatte als einzigen Aufrufer genau dieses `services/organizer.py` | P2 | **entfernt, `organizer.py`/`notification_service.py` nach expliziter Nutzer-Bestätigung** — alle 3 Dateien gelöscht (~1269 Zeilen), frisch verifiziert (keine Aufrufer, keine Testreferenzen — ein Treffer für „status_service" in `tests/test_enhanced_status_handler.py` erwies sich als reiner Substring-Zufallstreffer mit `callback_data="status_services"`, unrelated). Voller Regressionslauf: unverändert 15 Vorbestand-Fehler, 712 bestanden |
| TEST-016 | `services/downloader/download/formatters.py` (`ProgressFormatter`, live genutzt für ASCII-Logging in `download_utils.py`) hatte 0 Tests | P2 | **behoben** — 18 neue Tests in `tests/test_formatters.py`, alle Methoden abgedeckt (auch `playlist_start()`/`single_track_header()`, die aktuell keine externen Aufrufer haben — reine, zustandslose Formatierung ohne Seiteneffekte, bewusst mitgetestet statt entfernt). Kein neuer Bug gefunden. Voller Regressionslauf: 730 bestanden (vorher 712), unverändert 15 Vorbestand-Fehler |
| BUG-010 | `services/downloader/download/year_resolver.py` (`YearResolver`, live in `EnhancedDownloadProcessor._do_init()`, vorher 0 Tests) — beim Charakterisieren gefunden: `resolve_playlist_year()`s dritte Fallback-Quelle (`playlist_info["upload_date"]`) rief `_extract_year_from_text()` auf, das intern `YEAR_PATTERN` mit `(?!\d)`-Wortgrenze nutzt. Für das yt-dlp-Standardformat YYYYMMDD (durchgehende 8-stellige Ziffernfolge ohne Trennzeichen, z. B. `"20230815"`) schlägt dieser Regex-Ansatz **immer** fehl, da direkt nach dem Jahr weitere Ziffern (MMDD) folgen und die Wortgrenze das verhindert — live verifiziert: `_extract_year_from_text("20230815")` liefert `None`. Diese Fallback-Quelle war damit faktisch dauerhaft wirkungslos, obwohl der Docstring explizit "upload_date (YYYYMMDD)" als unterstütztes Format nennt. Die Schwesterlogik in `determine_dominant_year_from_entries()` (dieselbe Klasse) löst dasselbe Problem für dasselbe Feld bereits korrekt per direktem String-Slicing (`int(str(upload_date)[:4])`) | P1 | **behoben** — Quelle 3 in `resolve_playlist_year()` auf dieselbe direkte Slicing-Logik wie die bereits korrekte Schwesterstelle umgestellt. 23 neue Tests in `tests/test_year_resolver.py`, per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert. Dabei zusätzlich charakterisiert (nicht behoben, siehe unten): `_extract_year_from_text()` selbst behält dieses Verhalten bewusst bei (Freitext-Titel-Parsing braucht die Wortgrenze, um z. B. nicht Teile einer Video-ID als Jahr misszudeuten) — der Fix betrifft nur die Aufrufstelle, die fälschlich strukturierte YYYYMMDD-Daten durch die für Freitext gedachte Funktion schickte. Voller Regressionslauf: 753 bestanden (vorher 730), unverändert 15 Vorbestand-Fehler |
| ARTISTNORM-003 | Bei BUG-010 gefunden, nicht behoben: `YearResolver.YEAR_PATTERN` deckt nur Jahre 2000–2029 ab (`20[0-2]\d`), obwohl `YEAR_MAX = 2035` gesetzt ist. Ab dem Jahr 2030 würde `_extract_year_from_text()` (Freitext-Titel-Suche, z. B. "Song (2030 Remaster)") das Jahr nicht mehr erkennen, selbst wenn es laut `YEAR_MIN`/`YEAR_MAX` gültig wäre — live verifiziert. Aktuell (2026) noch nicht aktiv relevant | P2 | **nicht behoben, nur charakterisiert** — betrifft nur den Freitext-Regex-Pfad (Track-Felder mit direktem Jahr-Wert sind nicht betroffen, siehe `determine_dominant_year_from_entries()`s `release_year`/`year`-Felder). Reproduziert in `tests/test_year_resolver.py::TestExtractYearFromText::test_year_2030_and_beyond_is_not_matched_despite_year_max`. Da noch 3+ Jahre bis zur Relevanz, bewusst zurückgestellt statt einer spekulativen Regex-Änderung ohne akuten Bedarf |
| BUG-011 | `services/downloader/download/cache_manager.py` (`CacheManager`, live in `EnhancedDownloadProcessor._do_init()`, vorher 0 Tests, P0 — Cache-Verhalten) — beim Charakterisieren gefunden: `lookup_playlist_track()`s Stufe 2 (ArtistMap-Parsing-Fallback, greift wenn der direkte Cache-Lookup mit dem Playlist-Artist scheitert) rief `parsed.get("main_artist")`/`parsed["main_artist"]` auf das Ergebnis von `ArtistNormalizer.parse_youtube_title()` auf — diese Methode liefert aber `ParseResult`, ein **Dataclass ohne `.get()`-Methode**, kein Dict. Live verifiziert: `ParseResult(...).get("main_artist")` wirft `AttributeError: 'ParseResult' object has no attribute 'get'`. Der Fehler wurde vom umschließenden `except Exception` stillschweigend nur auf Debug-Level geloggt — **Stufe 2 hat seit dem Initial-Commit nie funktioniert**, jeder Aufruf fiel sofort auf einen Cache-Miss zurück, ohne dass dies in normalen Logs sichtbar wurde | P0 | **behoben** — Dict-Zugriffe (`.get()`/`[...]`) durch Attribut-Zugriff (`parsed.main_artist`/`parsed.title`) ersetzt, passend zum tatsächlichen `ParseResult`-Dataclass. 11 neue Tests in `tests/test_cache_manager.py`, darunter ein Regressionstest, der bewusst das **echte** `ParseResult`-Dataclass verwendet (nicht nur einen Mock mit `.get()`), um den exakten Bug-Mechanismus reproduzierbar zu machen — per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert. Voller Regressionslauf: 764 bestanden (vorher 753), unverändert 15 Vorbestand-Fehler |
| TEST-017 | `services/downloader/download/channel_router.py` (`ChannelRouter`, 5-stufiger Entscheidungsbaum für `dominant_artist`, live in `EnhancedDownloadProcessor._do_init()`) hatte 0 Tests | P1 | **behoben** — 14 neue Tests in `tests/test_channel_router.py`, alle 5 Entscheidungspfade (P1–P5) abgedeckt, inkl. `find_dominant_special_channel_from_entries()`/`_is_special_channel()`. Nutzt `mapping_dir_copy` (echte `special_channel.yaml`-Daten aus einer tmp-Kopie) statt erfundener Sonderkanal-Konfiguration. Kein neuer Bug gefunden. Voller Regressionslauf: 778 bestanden (vorher 764), unverändert 15 Vorbestand-Fehler |
| BUG-012 | `services/downloader/playlist_processor.py` (`PlaylistProcessor`, live in `EnhancedDownloadProcessor.__init__()`/`download_utils.py`, vorher 0 Tests) — beim Charakterisieren gefunden: `_extract_year_from_playlist()`s dritte/vierte Quelle (`playlist_info["upload_date"]`/`["release_date"]`) nutzte dieselbe `\b`-Wortgrenzen-Regex wie die Freitext-Quellen (Titel/Beschreibung). Für das yt-dlp-Standardformat YYYYMMDD (durchgehende Ziffernfolge, z. B. `"20180512"`) existiert zwischen Jahr und den folgenden Ziffern (MMDD) nie eine Wortgrenze — live verifiziert: `re.search(r"\b(19[5-9]\d\|20[0-2]\d)\b", "20180512")` liefert `None`. Diese beiden Quellen waren damit faktisch dauerhaft wirkungslos. Exakt derselbe Bug-Mechanismus wie BUG-010 (`YearResolver`), nur in einer anderen, parallelen Jahr-Ermittlungsfunktion — `_extract_year_from_playlist()`s Ergebnis fließt als `processed_playlist["year"]` in `YearResolver.resolve_playlist_year()`s Quelle 2 ein (`download_utils.py`), greift also nur, wenn Quelle 1 (Track-Einträge) komplett scheitert | P1 | **behoben** — Datums-Quellen (`upload_date`/`release_date`) von den Freitext-Quellen (`title`/`description`) getrennt; erstere nutzen jetzt direktes String-Slicing (`int(str(source)[:4])`) statt der für Freitext gedachten Wortgrenzen-Regex, analog zum bereits etablierten BUG-010-Fix. 4 neue Tests in `tests/test_playlist_processor.py::TestExtractYearFromPlaylist`, der Regressionstest per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert |
| LEGACY-012 | Bei der `playlist_processor.py`-Charakterisierung gefunden: modulweite Standalone-Funktion `_determine_dominant_year_from_playlist()` (ganze Counter-basierte Jahr-Bestimmungslogik, unverändert seit dem Initial-Commit) hatte 0 Aufrufer außerhalb der eigenen Datei — `download_utils.py` definiert stattdessen eine eigene, gleichnamige Ersatzfunktion, die an `YearResolver.determine_dominant_year_from_entries()` delegiert (den bereits bei BUG-010 getesteten Nachfolger) | P2 | **entfernt** — tote Funktion aus `playlist_processor.py` gelöscht, keine Aufrufer/Testreferenzen mehr vorhanden |
| TEST-018 | `services/downloader/playlist_processor.py` (`PlaylistProcessor`, 634 Zeilen, live in `download_utils.py:116`, vorher 0 Tests) hatte 0 Tests | P1 | **behoben** — 50 neue Tests in `tests/test_playlist_processor.py`, `__init__` (Erfolgs-/Fallback-Pfad separat via `monkeypatch` auf `ArtistNormalizer`), `determine_dominant_artist` (alle Pfade inkl. Override-Priorisierung), `_extract_collaboration_artists` (inkl. charakterisierter Lücke: „Feat." großgeschrieben wird nicht als Trenner erkannt, nur klein „feat."), `_clean_artist_name`, `_is_artist_known`, `_normalize_with_override_priority`, `clean_track_title`, `process_playlist_metadata`, `_extract_year_from_playlist`. Die meisten Tests umgehen `__init__` bewusst via `object.__new__` (Attribut-Injektion statt echtem Config-/Library-Scan-Aufbau). Siehe BUG-012/LEGACY-012 oben für die dabei gefundenen Funde |
| BUG-013 | `handlers/test_menu_handler.py` (`TestMenuHandler`, live in `RichMenuHandler` verdrahtet als Callback „test_unit", vorher 0 Tests) — beim Charakterisieren gefunden: `run_unit_tests()` rief `self._execute_test_run(update, "integration", timeout=600)` auf, also mit `test_type="integration"` statt `"unit"`. Der „Unit-Tests"-Button im Telegram-Admin-Testmenü führte dadurch tatsächlich das Integration-Test-Verzeichnis mit den Integration-spezifischen pytest-Flags (`-x` statt `--cov`) aus. Live per Codelesen verifiziert, kein Aufrufer hätte dies je richtiggestellt (nur `run_integration_tests()`/`run_performance_tests()` haben ihren jeweils korrekten Typ) | P2 | **behoben** — `"integration"` durch `"unit"` ersetzt. Regressionstest in `tests/test_test_menu_handler.py::TestBug013RunUnitTestsTypeRegression`, per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert. **Praktische Relevanz aktuell gering**: `tests/unit/`, `tests/integration/`, `tests/performance/` existieren im Repo nicht (die echte Suite liegt flach unter `tests/*.py`) — alle drei Menü-Buttons melden derzeit ohnehin „Keine Tests gefunden", unabhängig von diesem Bug. Diese Verzeichnisstruktur-Diskrepanz wird hier bewusst nur dokumentiert, nicht behoben (kein spekulativer Umbau der Test-Ablage ohne Nutzerentscheidung) |
| BUG-014 | Bei derselben Charakterisierung gefunden: `_parse_pytest_output()`s Extraktion fehlgeschlagener Testnamen nutzte `stripped_line.split("FAILED")[0].strip()` — das ist der Text **vor** dem Literal „FAILED", der bei realen pytest-Zeilen wie `"FAILED tests/test_x.py::test_y - AssertionError"` (Zeile beginnt mit „FAILED") immer ein Leerstring ist. `results["failed_tests"]` enthielt dadurch ausschließlich leere Einträge statt der tatsächlichen Testbezeichner — sichtbar in der Telegram-Ausgabe als leere Aufzählungspunkte unter „Fehlgeschlagene Tests". Live verifiziert: `"FAILED tests/test_foo.py::test_bar - X".split("FAILED")[0].strip()` liefert `''` | P2 | **behoben** — `split("FAILED")[0]` durch `split("FAILED", 1)[1]` ersetzt (Text nach dem Literal). Regressionstest `tests/test_test_menu_handler.py::TestParsePytestOutput::test_bug014_failed_test_name_extraction_regression`, per `git stash` gegen den Vor-Fix-Stand als fehlschlagend verifiziert |
| TEST-019 | `handlers/test_menu_handler.py` (`TestMenuHandler`, 686 Zeilen, Telegram-Admin-Testmenü, live in `RichMenuHandler`, vorher 0 Tests) hatte 0 Tests | P2 | **behoben** — 31 neue Tests in `tests/test_test_menu_handler.py`: `__init__`, alle drei `run_*_tests()`-Einstiegspunkte (inkl. BUG-013-Regression), `_execute_test_run` (Busy-Lock, fehlendes Testverzeichnis, Happy-Path, Timeout, unerwartete Exception, Lock-Freigabe via `finally`), `_parse_pytest_output` (inkl. BUG-014-Regression), `_show_test_results`, `show_test_details`, `show_coverage_report`, `show_all_test_results`, `run_all_tests`/`_run_test_type`. Async-Tests nutzen `asyncio.run()` statt `@pytest.mark.asyncio` (etabliertes Muster in diesem Repo, da `pytest-asyncio` in dieser Umgebung fehlt) |
| TEST-020 | `utils/podcast_rss_manager.py` (`PodcastRSSManager`/`PodcastRSSFeed`, live in `spotify_downloader.py:123`, Podcast-RSS-Fallback-Downloadpfad für Spotify-Shows, vorher 0 Tests) hatte 0 Tests | P2 | **behoben** — 36 neue Tests in `tests/test_podcast_rss_manager.py`, mit echten YAML-Dateien in `tmp_path` (keine Mocks für `yaml.safe_load`/`dump` — Regel 10: Mapping-Dateien sind Fachlogik): `_load_feeds()` (fehlende/leere/fehlerhafte Datei, deaktivierte/ungültige Feeds werden übersprungen), `get_feed`/`get_feed_by_name` (exakter Match + bidirektionales Teilstring-Fuzzy-Matching), `has_feed`, `get_feed_url`, `get_all_feeds` (Kopie, kein Leck ins interne Dict), `get_enabled_feeds`, `get_statistics`, `reload`, `add_feed` (Datei-Erstellung, Erhalt bestehender Einträge, Persistenz über Neuinstanziierung, Schreibfehler-Pfad). Kein neuer Bug gefunden — reine Charakterisierung. `add_feed`/`get_all_feeds`/`get_enabled_feeds`/`get_statistics`/`has_feed`/`get_feed_url`/`reload` haben aktuell keine Aufrufer außerhalb dieser Datei (nur `get_feed`/`get_feed_by_name` werden von `spotify_downloader.py` genutzt), sind aber Teil der zusammenhängenden öffentlichen Manager-API (`add_feed` persistiert dauerhaft in die YAML-Datei) — analog zu TEST-016 bewusst mitgetestet statt als Legacy behandelt |
| ARCH-002 | **Wichtiger Architektur-Befund** beim Charakterisieren von `services/downloader/utils/file_utils.py` (`FileUtils`, SingletonMixin) gefunden: die Klasse wird an mehreren Stellen konstruiert (`downloader.py`, `download_utils.py:112`, `klassen/download_handler.py:214`) und als `file_utils`-Parameter durch die **gesamte** Download-Pipeline durchgereicht (u. a. bis in `EnhancedMetadataProcessor.process_single_track()`), aber repo-weit per Grep verifiziert: **keine** ihrer 5 öffentlichen Methoden (`verify_file`, `safe_rename`, `clean_temp_files`, `create_dir`, `sanitize_filename`) und auch nicht das `.library_dir`-Attribut werden irgendwo tatsächlich aufgerufen/gelesen — `process_single_track()` nimmt `file_utils` sogar entgegen, ohne es im gesamten Methodenkörper zu referenzieren. Reale Datei-Sanitisierung/-Umbenennung läuft stattdessen über die eigenständigen, bereits genutzten Funktionen in `utils/helpers.py` (`verify_file`/`safe_rename`/`sanitize_filename`), importiert von `utils/filenamefixer.py::FilenameFixerTool`. Damit ist `FileUtils` funktional komplett "totes Gewicht", das nur die Illusion von Nutzung erzeugt (Konstruktion + Durchreichen ohne jeden Effekt) — dieselbe Kategorie wie BUG-008 (Navidrome-Adapter), hier aber auf eine ganze, breit verdrahtete Klasse statt eine einzelne Methode bezogen. Besonders relevant: `clean_temp_files()` (Aufräumen alter Temp-Dateien >1h) läuft dadurch **nie** — potenzielles unbegrenztes Wachstum des Download-Verzeichnisses über Zeit, analog zur Kategorie der bereits behobenen REL-005-Ressourcen-Limit-Funde | P1 | **nicht behoben, nur charakterisiert und dokumentiert** — 23 neue Tests in `tests/test_file_utils.py` (echte Fachlogik, könnte jederzeit reaktiviert werden), aber bewusst **keine** Verdrahtung/Aktivierung vorgenommen (z. B. `clean_temp_files()` periodisch aufrufen), da dies eine echte Verhaltensänderung der laufenden Produktions-Pipeline wäre (Regel 2: bestehendes Verhalten nicht unbewusst ändern) und eine explizite Nutzerentscheidung braucht, ob/wie das reaktiviert werden soll (z. B. Cronjob für Temp-Cleanup, oder bewusstes Entfernen als bestätigtes Legacy analog LEGACY-011) |
| TEST-021 | `services/downloader/utils/file_utils.py` (`FileUtils`, 256 Zeilen, siehe ARCH-002, vorher 0 Tests) hatte 0 Tests | P1 | **behoben** — 23 neue Tests in `tests/test_file_utils.py`: `verify_file` (fehlende/gültige/leere-und-gelöschte Datei), `safe_rename` (fehlende Quelle, Zielverzeichnis-Erstellung, Überschreiben vorhandener Zieldatei), `clean_temp_files` (alte Datei gelöscht, neue behalten, Verzeichnisse übersprungen, fehlendes Verzeichnis crasht nicht), `create_dir` (Erstellung, No-op bei Existenz, `base_path`, `FileProcessingError` bei Fehler), `sanitize_filename` (`None`, normale Namen, illegale Zeichen, Längenkürzung auf `Config.MAX_FILENAME_LENGTH`, ft→feat-Normalisierung inkl. Regressionsschutz für „trifft"/„Kraftklub" analog ARTISTNORM-001/002, Whitespace-Kollabierung, Exception-Fallback). **Lektion beim Schreiben**: ein erster Exception-Test patchte `unicodedata.normalize` global — das ist dasselbe geteilte Stdlib-Modul, das pytest selbst intern für Terminal-Breitenberechnung nutzt, und riss die gesamte Testsitzung mit einem `pytest-INTERNALERROR` ab; korrigiert auf einen Patch des modul-eigenen `EXTRA_SPACES_PATTERN`-Namens (kompilierte Regex-Objekte sind zudem unveränderlich, daher Ersetzen der Modul-Referenz statt eines Attributs auf dem Objekt) |
...
- [x] ARCH-001 behoben (Analyse-Teil) — alle vier großen Orchestratoren dokumentiert (Verantwortlichkeiten/Schnittstellen/Aufrufer), siehe [`docs/archive/arch/MusicBot_ARCH-001_Orchestrators.md`](arch/MusicBot_ARCH-001_Orchestrators.md)
...
- [x] TEST-013 behoben — `utils/youtube_parser.py` (P0-Pfad, vorher 0 Tests) charakterisiert, 43 neue Tests, kein neuer Bug gefunden
- [x] LEGACY-008 behoben — tote `handlers/message_handler.py`/`handlers/telegram_handler.py` entfernt (Fortsetzung der systematischen Ungetestet-Prüfung)
- [x] TEST-014 behoben — `LyricsProcessor` (P0-Metadaten-Sub-Prozessor, vorher 0 Tests) charakterisiert, 13 neue Tests, kein neuer Bug gefunden
- [x] BUG-008 behoben — „Navidrome Scan"-Button war end-to-end unerreichbar (nie instanziierter `NavidromeAdapter`), jetzt auf die bereits getestete `NavidromeAPI.execute_scan()` umgestellt, 4 neue Tests
- [x] LEGACY-009 behoben — totes `utils/yt_utils.py` entfernt
- [x] TEST-015 behoben — `BotRestartHandler` (live, administrativ riskant, vorher 0 Tests) charakterisiert, 12 neue Tests, `cancel_restart()`-Admin-Check-Frage als sicher bestätigt
- [x] BUG-009 behoben — `ProgressTracker.cleanup()` fehlte komplett (`AttributeError` bei Verdrahtung von `init_tracker()`), No-Op-Methode ergänzt, 16 neue Tests
- [x] LEGACY-010 behoben — totes `utils/organizer.py` entfernt, tote Teile aus `utils/file_ops.py` entfernt, 6 neue Tests für den verbliebenen Live-Code
- [x] LEGACY-011 behoben — `services/status_service.py` (tot) sowie `services/organizer.py`/`services/notification_service.py` (nach Nutzer-Bestätigung: nicht mehr manuell genutzt) entfernt, ~1269 Zeilen
- [x] TEST-016 behoben — `ProgressFormatter` (live, vorher 0 Tests) charakterisiert, 18 neue Tests, kein neuer Bug
- [x] BUG-010 behoben — `YearResolver.resolve_playlist_year()`s upload_date-Fallback war für das yt-dlp-Standardformat YYYYMMDD faktisch immer wirkungslos, 23 neue Tests
- [x] ARTISTNORM-003 charakterisiert, nicht behoben — `YEAR_PATTERN` deckt nur Jahre bis 2029 ab trotz `YEAR_MAX=2035`, noch nicht aktiv relevant (aktuelles Jahr 2026)
- [x] BUG-011 behoben (P0) — `CacheManager`s Stufe-2-Cache-Lookup hat seit dem Initial-Commit nie funktioniert (`ParseResult`-Dataclass fälschlich wie ein Dict behandelt), 11 neue Tests
- [x] TEST-017 behoben — `ChannelRouter` (live, vorher 0 Tests) charakterisiert, 14 neue Tests, kein neuer Bug
- [x] TEST-018/BUG-012/LEGACY-012 behoben — `PlaylistProcessor` (live, vorher 0 Tests) charakterisiert, 50 neue Tests, 1 neuer Bug (`_extract_year_from_playlist()`s upload_date/release_date-Quellen wie BUG-010 wirkungslos) behoben, 1 tote Standalone-Funktion entfernt
- [x] TEST-019/BUG-013/BUG-014 behoben — `TestMenuHandler` (live, vorher 0 Tests) charakterisiert, 31 neue Tests, 2 neue Bugs behoben (Unit-Test-Button führte Integration-Tests aus; FAILED-Testnamen-Extraktion lieferte immer Leerstrings). Zusätzlich dokumentiert (nicht behoben): `tests/unit/`/`tests/integration/`/`tests/performance/` existieren im Repo nicht, das gesamte Testmenü-Feature ist dadurch aktuell praktisch wirkungslos — separates Architektur-Thema, keine Nutzerentscheidung dazu eingeholt
- [x] TEST-020 behoben — `PodcastRSSManager`/`PodcastRSSFeed` (live, vorher 0 Tests) charakterisiert, 36 neue Tests mit echten YAML-Dateien, kein neuer Bug gefunden
- [x] TEST-021/ARCH-002 behoben — `FileUtils` charakterisiert, 23 neue Tests. Wichtiger Architektur-Befund dokumentiert (nicht behoben, braucht Nutzerentscheidung): die Klasse wird breit durch die Download-Pipeline durchgereicht, aber KEINE ihrer 5 Methoden wird je aufgerufen — `clean_temp_files()` läuft dadurch nie, potenzielles unbegrenztes Temp-Verzeichnis-Wachstum
- [x] **Ungetestet-Liste vollständig abgearbeitet** (TEST-013 bis TEST-021). Die beiden reinen ABC/Protocol-`interfaces.py`-Dateien haben keine Implementierung und wurden bewusst übersprungen. `advanced_podcast_finder.py` bleibt bewusst unangetastet (eigenständiges manuelles CLI-Tool, kein Bot-Aufrufer, aber unklar ob vom Nutzer aktiv verwendet — nicht ohne Rückfrage löschen). Offene Entscheidung: ARCH-002 (`FileUtils` reaktivieren oder als bestätigtes Legacy entfernen?)
- [x] ARCH-003 Phase 1 (Analyse) abgeschlossen — vollständiges Service Inventory, Dependency-Graph, priorisierte Problem List (P-1 bis P-14) und Zielstruktur-Vorschlag für `services/` in `docs/archive/arch/MusicBot_ARCH-003_Services_Phase1_Analyse.md`. Wichtigster Fund: der YouTube-Download-Kern (`download/*.py`, `metadata/*.py`) ist bereits sauber geschichtet (echte DI, funktionierendes Protocol-Port-System) und dient als Referenz; die eigentlichen Probleme liegen in `statistik_service.py` (God-Service), Telegram-Kopplung in `services/` (`progress_tracker.py`, `download_result_reporter.py`), einer zweiten, unabhängigen Orchestrierungs-Implementierung für Spotify (parallel zu `download_utils.py`) und mehreren komplett unbenutzten, aber breit verdrahteten Komponenten
- [x] ARCH-003 Phase 2/3 (risikoarmer Kern, Branch `arch/services-migration`) umgesetzt — P-4 (tote Telegram-Handler-Funktionen in `error_handler.py` entfernt, umbenannt zu `errors.py`), P-5 (kaputte tote Methode in `downloader.py` entfernt), P-8 (`PlaylistProcessor`/`SpotifyDownloader`: `config`/`rss_manager` optional injizierbar, Default-Verhalten unverändert), P-9 (Cookie-Pfad aus `Config.COOKIES_FILE` statt hartkodiert), P-12 (tote Fake-Interfaces `IDownloaderConfig`/`IDownloader` entfernt), P-13 (zwei tote Legacy-Wrapper-Funktionen + `TrackDownloadState` entfernt). 6 neue Tests, Regressionslauf: 920 bestanden (vorher 914), unverändert 15 Vorbestand-Fehler. Bewusst zurückgestellt (Nutzerentscheidung „nur Kern"): P-1/P-2/P-3/P-6/P-11/P-14 — siehe Dokument für Details
- [x] ARCH-003 P-6 umgesetzt (Branch `arch/services-migration`) — `StatistikService` (573 Zeilen, God-Service) in 4 fokussierte Klassen unter `services/statistik/` aufgeteilt: `PlayHistoryRepository` (Datei-Persistenz), `PlayHistoryPoller` (Navidrome-Hintergrund-Polling, `NavidromeAPI` jetzt injiziert statt selbst konstruiert), `StatisticsCalculator` (reine Business-Logik), `ChartRenderer` (matplotlib, einziger Ort im Repo mit dieser Abhängigkeit). `statistik_service.py` ist jetzt eine dünne, klar gekennzeichnete Fassade mit exakt unveränderter öffentlicher API (inkl. der extern getesteten „privaten" Methoden und der monkeypatchbaren Klassenattribute `CHARTS_DIR`/`USER_HISTORY_DIR`) — `bot.py`/`handlers/mugge_statistik_handler.py` komplett unverändert. 40 neue isolierte Tests für die 4 neuen Klassen, alle 29 bestehenden Statistik-Tests unverändert grün. Regressionslauf: 960 bestanden (vorher 920), unverändert 15 Vorbestand-Fehler
- [x] ARCH-004 P-3 Option B umgesetzt (Branch `arch/services-migration`) — gemeinsame Integrationsschicht `services/downloader/utils/metadata_result_translator.py` extrahiert (`call_process_single_track`, `build_playlist_track_result`, `build_single_track_result`, `merge_metadata_result_into_dict`). Ersetzt die zuvor dreifach duplizierte `process_single_track()`-Aufruf+Ergebnis-Übersetzung in `download_utils.py::_process_track_metadata()`/`_process_single_download()` (YouTube) und `klassen/download_handler.py::_process_single_download_result()` (Spotify). Vorab exakte Feld-für-Feld-Charakterisierung aller drei Stellen (nicht nur YT-vs-Spotify — auch die beiden YouTube-Pfade wichen bereits untereinander ab, u. a. `year`/`track_number`/`is_duplicate`/`library_path`-Stringifizierung bei `None`) — keine der gefundenen Inkonsistenzen wurde angeglichen, alle als Folgeentscheidungen dokumentiert. `EnhancedMetadataProcessor` und der Already-Processed-Vertrag blieben unverändert. 47 neue Tests (29 Regressionstests VOR der Extraktion + 18 für die neue Integrationsschicht), alle unverändert grün nach dem Einbau — kein bestehender Test musste angepasst werden. Regressionslauf: 1007 bestanden (vorher 989), unverändert 15 Vorbestand-Fehler. Siehe `docs/archive/arch/MusicBot_ARCH-004_P3_Orchestrierungs_Analyse.md`
- [x] ARCH-004 P-3 Folgeentscheidungen bewertet (Entscheidungsbericht FIX NOW/DEFER, siehe ARCH-004 Abschnitt 7) und zwei Bugs gefixt: `is_duplicate` wird im YT-Single-Pfad jetzt aus dem echten `MetadataResult` übernommen (vorher immer `False` — täuschte im Telegram-Report „kein Duplikat" vor); `library_path` wird im YT-Playlist-Pfad bei `None` nicht mehr zum truthy-String `"None"` (konnte den Already-Processed-Vertrag fälschlich auslösen bzw. in `cache_manager.py` einen ungültigen `Path("None")` erzeugen). Zurückgestellt (DEFER): Already-Processed-Vertrag, fehlender `enhanced_processor_ref` im Spotify-Pfad, zwei unabhängige Podcast-Erkennungsmechanismen. Regressionstests angepasst, Regressionslauf: 1008 bestanden, unverändert 15 Vorbestand-Fehler
- [x] ARCH-003 P-11 umgesetzt — `klassen/genius_client.py`, `klassen/lastfm_client.py`, `klassen/musicbrainz_client.py` (reine externe API-Adapter, kein Handler-/Telegram-Bezug) nach neuem `services/clients/` verschoben, je eigener Commit. Ursprünglich nur für `genius_client` formuliert; vertiefte Analyse zeigte strukturell identische Geschwister — Nutzer entschied sich für alle drei gemeinsam. Neue Architekturregel: `services/clients/` enthält ausschließlich externe Integrationsadapter, keine Fachlogik. `klassen/` enthält jetzt nur noch `download_handler.py` (Orchestrator). Reine Verschiebung + Import-Updates (Produktionscode + Tests inkl. `mock.patch`-Ziele), keine Verhaltensänderung. Regressionslauf: 1008 bestanden, unverändert 15 Vorbestand-Fehler
- [x] ARCH-003 P-1 umgesetzt (Branch `arch/p1-fileutils-dead-wiring`) — `services/downloader/utils/file_utils.py` (`FileUtils`, `SingletonMixin`, totes Gewicht laut ARCH-002-Befund: keine ihrer 5 Methoden wurde je aufgerufen) komplett entfernt. Nutzerentscheidung unter drei vorgelegten Optionen: „nur entfernen", kein Reaktivierungsversuch. Der reine Pass-Through-Parameter `file_utils` durch die gesamte Kette entfernt: `enhanced_metadata_processor.py::process_single_track()`, `metadata_result_translator.py::call_process_single_track()`, alle drei Pipeline-Funktionen in `download_utils.py`, `klassen/download_handler.py`, `services/downloader/downloader.py` (`YoutubeDownloader`, Attribut war dort ebenfalls nie gelesen), `MetadataEnricher`-Protocol in `download/interfaces.py` konsistent nachgezogen. `tests/test_file_utils.py` (23 Tests) gelöscht, 6 weitere Testdateien angepasst (Parameter aus Aufrufen entfernt). Regressionslauf: 985 bestanden (vorher 1008, Differenz = gelöschte Testdatei), unverändert 15 Vorbestand-Fehler. Temp-Verzeichnis-Bereinigung (`clean_temp_files()` lief nie, dokumentiertes operationelles Risiko) bleibt bewusst als eigene, zurückgestellte Folgeentscheidung offen — Reaktivierung wäre echte Verhaltensänderung und war nicht Teil dieser Freigabe. Siehe `docs/archive/arch/MusicBot_ARCH-003_Services_Phase1_Analyse.md`
- [x] ARCH-005 Temp-Cleanup umgesetzt (Branch `arch/arch-005-temp-cleanup`) — Folgeentscheidung aus ARCH-003/P-1. Neues Modul `services/downloader/utils/download_artifact_cleanup.py` mit zwei Funktionen statt Reaktivierung der alten `clean_temp_files()` (Name war irreführend, suggerierte `TEMP_DIR` statt des tatsächlich betroffenen `DOWNLOAD_DIR`; `TEMP_DIR` hat repo-weit keine einzige echte Datei-Operation, bleibt unangetastet). **Strategie C (primär):** `cleanup_single_download_artifact()` — gezielter Cleanup im Fehlerpfad von `EnhancedMetadataProcessor.process_single_track()`, dem einen zentralen Ort, durch den alle drei Pipelines (YT-Playlist/YT-Single/Spotify) garantiert laufen (verifiziert: ein einziger `try`/`except`-Block ohne "weiche" Zwischen-Returns). Löscht nur, wenn die Datei noch existiert und nachweislich innerhalb `DOWNLOAD_DIR` liegt — automatischer No-op, falls `move_to_library()` vor dem Fehler bereits gelaufen ist. **Strategie A (Fallback):** `cleanup_download_artifacts()` — konservativer Sweep beim Bot-Start (`bot.py`, vor `bot_runner.initialize()`, garantiert kein aktiver Download), Endungs-Whitelist statt Namens-Pattern, 24h-Altersgrenze (statt der alten, nicht mehr verwendeten 1h-Grenze), `.part`/`.ytdl`-Dateien werden nie gelöscht (explizite Nutzeranforderung), nicht rekursiv. Periodische Hintergrundbereinigung (Option B) bewusst nicht umgesetzt. 20 neue Tests (18 isoliert für das neue Modul, 2 End-to-End über die echte Pipeline inkl. erzwungenem `move_to_library()`-Fehler). Regressionslauf: 1005 bestanden (vorher 985), unverändert 15 Vorbestand-Fehler. Siehe `docs/archive/arch/MusicBot_ARCH-005_TempCleanup.md`
- [x] ARCH-003 P-14 umgesetzt (Branch `arch/p14-advanced-podcast-finder`) — `services/downloader/advanced_podcast_finder.py` (833 Zeilen, eigenständiges CLI-Recherche-Tool für Podcast-RSS-Feeds, 0 Importe/Aufrufer/Tests im gesamten Repo) komplett entfernt. Vor der Entscheidung vollständig auf Consumer, Tests, Deployment-Referenzen (Shell/Cron/systemd/Docker — existieren im Repo ohnehin nicht) und Doku geprüft — alle 0. Einziger Hinweis auf frühere Nutzung: Default-Wert im interaktiven Demo-Modus entsprach dem einen Eintrag in `mapping/podcast_rss_feeds.yaml`; die repo-seitige Analyse konnte aktuelle manuelle Terminal-Nutzung technisch nicht klären (Präzedenzfall LEGACY-011), Nutzer bestätigte auf Nachfrage explizit „nicht mehr in Gebrauch". Keine Migration nach `tools/`, direkte Entfernung. Regressionslauf: 1005 bestanden, exakt unverändert gegenüber dem Stand vor der Entfernung (15 Vorbestand-Fehler) — wie für ein komplett unverdrahtetes Modul erwartet. Siehe `docs/archive/arch/MusicBot_ARCH-003_Services_Phase1_Analyse.md`
- [x] ARCH-006 abgeschlossen — vollständiger Import-/Dependency-Graph für `services/` (39 Python-Dateien) als P-2-Vorbereitung. Ergebnis: P-2-Scope vollständig kartiert, genau zwei Fundstellen (`download_result_reporter.py`, `progress_tracker.py`), keine Zyklen, keine Legacy-Imports, keine `services/`→`klassen/`-Abhängigkeiten. Zusätzlich gefunden, aber explizit nicht P-2-relevant: `statistik_service.py` importiert `api.navidrome_api.NavidromeAPI` außerhalb der P-11-Konvention — eigener späterer Kandidat. Siehe `docs/archive/arch/MusicBot_ARCH-006_P2_Dependency_Graph.md`
- [x] ARCH-003 P-2 umgesetzt (Branch `arch/p2-telegram-decoupling`) — Design-Analyse (ARCH-007) plus Umsetzung. Wichtigster Zusatzbefund: `progress_tracker.py`s Telegram-Versand-Pfad (`update_progress()`) hatte 0 Aufrufer im Produktionscode, war bereits funktional tot. `download_result_reporter.py`: `send_final_summary()`/`send_playlist_direct_summary()` → `build_final_summary_message()`/`build_playlist_summary_message()`, geben nur noch Text zurück, `TelegramError`-Import entfernt. `progress_tracker.py`: `update_progress()` → `compute_progress_message()` (`Optional[str]`), `update`-/`status_message`-Parameter aus Konstruktor entfernt. `klassen/download_handler.py`: neue Hilfsmethode `_send_report_message()` bündelt das vorher 3× duplizierte Send-Muster (status_msg-Fallback, `TelegramError`-Fang), alle Log-Texte 1:1 erhalten. `download_utils.py::init_tracker()` `update_object`-Parameter entfernt (0 Aufrufer). 6 neue Tests für `_send_report_message()`, bestehende Tests auf Rückgabewert-Assertions umgestellt. Regressionslauf: 1007 bestanden (vorher 1005), unverändert 15 Vorbestand-Fehler. `services/` besitzt jetzt keine Telegram-Abhängigkeit mehr. `api/navidrome_api.py` bewusst nicht angefasst (separate Entscheidung). Siehe `docs/archive/arch/MusicBot_ARCH-007_P2_Entkopplungsvorschlag.md`
- [x] ARCH-008/ARCH-009 abgeschlossen (Analyse + Phase 1 + Phase 2, Branch `arch/arch-009-phase2-dead-navidrome-methods`) — `api/navidrome_api.py` ist kein reiner Integrationsadapter, vermischt API-Kommunikation, Telegram-MarkdownV2-Formatierung und Docker-Subprocess-Steuerung (`execute_scan()`). Bestandsaufnahme ergab: 7 von 12 öffentlichen Methoden hatten 0 Produktions-Consumer. Nutzerentscheidung: sechs davon entfernt (`format_full_status_message()`, `format_rescan_status_message()`, `format_web_interface_url_message()`, `get_full_server_info()`, `get_scan_status()`, `test_api()` — zusammen ~220 Zeilen), `check_connection()` bewusst behalten (dokumentierter BUG-007-Beleg für bewusst zurückgestellte, geplante Nutzung). Vor der Entfernung alle Consumer repo-weit nochmals verifiziert. 4 zugehörige Charakterisierungstests entfernt (`TestGetScanStatus`/`TestGetFullServerInfo`), keine anderen Tests/Funktionalität verändert. Regressionslauf: 1003 bestanden (vorher 1007, Differenz = exakt die entfernten Tests), unverändert 15 Vorbestand-Fehler. Keine Verschiebung nach `services/clients/`, keine DI-Umstellung, keine Telegram-Entkopplung, `execute_scan()` unverändert — bewusst zurückgestellt als Phase 3/4. Siehe `docs/archive/arch/MusicBot_ARCH-008_Navidrome_Adapter_Analyse.md`, `docs/archive/arch/MusicBot_ARCH-009_Navidrome_Migrationsplanung.md`, `docs/archive/arch/MusicBot_ARCH-009_Phase1_Bestandsaufnahme.md`
- [x] ARCH-009 Phase 3 bis Folgeumsetzung abgeschlossen (mehrere Branches/PRs, zuletzt `arch/arch-009-navidrome-scan-trigger-to-utils`, Merge-Commit `d07cddb`) — die in der vorherigen Zeile zurückgestellte `execute_scan()`-Auslagerung samt späterer Adapter-Zielposition/DI wurde vollständig umgesetzt: Subprocess-Steuerung nach `NavidromeScanTrigger` extrahiert (Phase 4), Telegram-Formatierung aus dem Adapter entfernt (Phase 5), `NavidromeAPI` auf DI umgestellt (Phase 7), reiner API-Adapter nach `services/clients/navidrome_api.py` verschoben (Phase 8), die historische `execute_scan()`-Bridge vollständig eliminiert (Phase 9), `NavidromeScanTrigger` abschließend nach `utils/navidrome_scan_trigger.py` verschoben und `api/` komplett entfernt (Folgeumsetzung). Ergebnis per Post-ARCH-009-Audit verifiziert: 0 verbleibende funktionale Referenzen auf `api.*`, keine Telegram-Kopplung in `services/clients/`/`utils/`. Siehe `docs/archive/arch/MusicBot_ARCH-009_Navidrome_Migration_Roadmap.md` (Status: ABGESCHLOSSEN) und `docs/archive/post-arch/MusicBot_POST-ARCH-009_Audit.md`
- [x] POST-ARCH-010/011 `DuplicateEntry`-Migration umgesetzt (Branch `post-arch-010-011/duplicate-entry-migration`) — die in ARCH-007 Abschnitt 4 offen gelassene `handlers.duplicate_handler.DuplicateEntry`-Abhängigkeit (letzte verbliebene `services/*→handlers/*`-Schichtverletzung) aufgelöst: reiner `@dataclass`-Datencontainer ohne Telegram-Bezug nach neuem `services/downloader/models.py` verschoben. Consumer aktualisiert: `handlers/duplicate_handler.py` (importiert die Klasse jetzt zurück, `DuplicateCache`/`EnhancedDuplicateHandler` unverändert), `services/downloader/download_result_reporter.py`, `klassen/download_handler.py`, `tests/test_download_result_reporter.py`, `tests/test_duplicate_handler.py`. JSON-Cache-Format, Verhalten und Telegram-Logik unverändert. Regressionslauf: 1009 bestanden, unverändert 15 Vorbestand-Fehler. Siehe `docs/archive/post-arch/MusicBot_POST-ARCH-010_011_DuplicateEntry_Analyse.md`
- [ ] Zielarchitektur schrittweise weiter umsetzen (die drei zurückgestellten DEFER-Punkte aus ARCH-004 Abschnitt 7 und die in ARCH-005 Abschnitt 5 dokumentierten Restrisiken — je eigene Nutzerentscheidung nötig). Neuer, vom Post-ARCH-009-Audit empfohlener nächster Kandidat (P-1): `handlers/admin/bot_restart_handler.py::_trigger_restart()` nach `utils/` extrahieren — siehe `docs/archive/post-arch/MusicBot_POST-ARCH-009_Audit.md`, Abschnitt 5
