...
- **Nebenbefunde:**
  ...
  - **Inzwischen behoben (ENCAP-001):** `rich_menu_handler.py:382-385` (`RichMenuSystem.menu_registry`-Durchgriff → `add_child_menu_item()`) und `bot.py:287-296` (`genius_client`-Durchgriff → `EnhancedMetadataProcessor.aclose()`). Dabei zusätzlich eine verwaiste Backup-Datei `genre_processor.py.blak` entfernt und entdeckt, dass `EnhancedMetadataProcessor.cleanup()` (sync) im Shutdown-Pfad nie aufgerufen wird (dokumentiert, nicht gefixt, Teil des ENCAP-001-Eintrags in der Baseline).
  - **Inzwischen behoben (AUTOLEARN-001):** `download_utils.py:739/934` → `auto_learn_manager.learn_artist()` war kein reiner Kapselungsfall, sondern deckte eine echte Lücke auf: die interne Sonderkanal-Prüfung in `process_single_track()` kannte nur 2 hartcodierte Kanalnamen, während der externe (jetzt entfernte) Aufruf die vollständige `special_channel.yaml` prüfte. Die interne Prüfung wurde entsprechend erweitert (rein additiv), die externen Aufrufe entfernt. Dabei ein eigenständiger, nicht behobener Nebenbefund: `ArtistNormalizer.normalize()` mangelt Namen mit "ft"-Substring (siehe ARTISTNORM-001 in der Baseline).
