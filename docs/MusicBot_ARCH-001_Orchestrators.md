...
- **Nebenbefunde:**
  ...
  - **Weiterhin offen, jetzt als AUTOLEARN-001 in der Baseline:** `download_utils.py:739/934` → `auto_learn_manager.learn_artist()` ist kein reiner Kapselungsfall mehr, sondern ein potenziell redundanter zweiter Aufruf derselben Lern-Logik mit leicht abweichenden Bedingungen (idempotent/harmlos, aber Absicht unklar) — verdient eine eigene Untersuchung statt eines mechanischen Delegate-Fixes.
