...
Genre-Bestimmung. Primary/Secondary werden bei jeder neuen Beobachtung
per Mehrheitsvotum über die letzten 10 Beobachtungen neu abgeleitet
(`_aggregate_genre_observations()`, `auto_learn.py:82`) — explizit
**kein** "last value wins".
...
```bash
python3 -m pytest tests/test_genre_processor*.py tests/test_auto_learn*.py tests/test_artist_normalizer.py tests/test_mapping_yaml_integrity.py -q
```
