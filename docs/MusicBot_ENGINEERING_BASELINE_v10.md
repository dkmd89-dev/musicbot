...
| 2026-09-09 | **Telegram-UI für ⚪ akzeptierte Findings + Unaccept** — direkt im Anschluss an die Closure Phase (nach CLI-Test des Nutzers gegen die echte Registry). `handlers/library_health_review_handler.py`: `handle_accepted_list`/`handle_accepted_category`/`handle_accepted_show`/`handle_unaccept`. `rich_menu_system.py`-Review-Dispatcher: `review:accepted`/`review:acccode:<code>`/`review:accshow:<fid>`/`review:unaccept:<fid>`. Übersicht zeigt „⚪ Akzeptierte Findings (N)" nur wenn vorhanden → Liste nach Code → Detail mit Grund/Scan-Status → „↩️ Reaktivieren" (`unaccept_finding()` → OPEN, Stale-Revalidierung wie bei allen Statusänderungen). Rein lesend bis zum expliziten Tap; stale mit ⚠️. Kein neues Menüsystem, gleiche `FindingsRegistry`-API + Admin-Gating. `test_library_health_review_handler.py` +11, `test_rich_menu_review.py` +8. | (Branch `feat/library-review-accepted-telegram`, noch nicht gemergt) | thematisch grün (`-k "review or findings or rich_menu"` 183); volle Suite beim Nutzer |
