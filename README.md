...
| `services/library_repair/` | Leitet aus dem Health-Report konkrete Reparaturaktionen ab und führt sie aus: Planner (Issue-Code → Repair-Level, read-only) + 7 Executoren (Tag-Fixes, Renames, Cover, Album-Cover-Vereinheitlichung, MusicBrainz-IDs/ISRC, volle Neuverarbeitung, ReplayGain) mit Per-Datei-Backup außerhalb der Library, Audio-Essenz-Verifikation, Rollback bei Fehler und Append-Only-Journal; Duplicate-Auflösung dockt separat an `scripts/resolve_duplicates.py` an. Alle Executoren gegen die Produktions-Library gelaufen (Stand 2026-09-04). Details: [`docs/LIBRARY_REPAIR.md`](docs/LIBRARY_REPAIR.md) |
