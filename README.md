...
Historisch organisch gewachsenes Hobbyprojekt — siehe [`CLAUDE.md`](CLAUDE.md) für die Engineering-Leitlinien und [`docs/MusicBot_ENGINEERING_BASELINE_v9.md`](docs/MusicBot_ENGINEERING_BASELINE_v9.md) für den eingefrorenen technischen Referenzpunkt (Architektur, Testabdeckung, Security, Technical Debt). Änderungen seit dem v9-Freeze werden in [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT) mitgeschrieben; der laufende Stand aller offenen/zurückgestellten Punkte steht in [`docs/FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md).
...
| `services/library_repair/` | Leitet aus dem Health-Report konkrete Reparaturaktionen ab und führt sie aus: Planner (Issue-Code → Repair-Level, read-only) + 7 Executoren (Tag-Fixes, Renames, Cover, Album-Cover-Vereinheitlichung, MusicBrainz-IDs/ISRC, volle Neuverarbeitung, ReplayGain) mit Per-Datei-Backup außerhalb der Library, Audio-Essenz-Verifikation, Rollback bei Fehler und Append-Only-Journal; Duplicate-Auflösung dockt separat an `scripts/resolve_duplicates.py` an. Automatischer Navidrome-Scan nach `--apply` mit echter Änderung. `doctor_runner.py` orchestriert Health-Scan + SAFE_AUTOMATIC-Apply als Subprozesse für den Telegram-Menüpunkt „MusicBot Doctor". Alle Executoren gegen die Produktions-Library gelaufen (Stand 2026-09-04). Production Audit (2026-09-08) unabhängig verifiziert: `apply_level2()` bindet SUCCESS jetzt an das je Issue-Code relevante Zielfeld statt an „irgendein Feld geändert", vier zuvor tote `EXTERNAL_METADATA`-Codes korrekt umklassifiziert. Details: [`docs/LIBRARY_REPAIR.md`](docs/LIBRARY_REPAIR.md), [`docs/audits/LIBRARY_REPAIR_P1_P2_P3_CLOSURE_AUDIT_2026-09-08.md`](docs/audits/LIBRARY_REPAIR_P1_P2_P3_CLOSURE_AUDIT_2026-09-08.md) |
...
| `tests/` | 2920 Tests (pytest), 0 bekannte Fehlschläge, 1 umgebungsbedingt übersprungen (Stand 2026-09-08, nach der Artist-Identity-Resolution-Migration — Details: [`docs/audits/ARTIST_IDENTITY_RESOLUTION_MIGRATION_2026-09-08.md`](docs/audits/ARTIST_IDENTITY_RESOLUTION_MIGRATION_2026-09-08.md)) — Characterization-Tests für die Produktionsklassen, laufender Stand: [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT) |
| `docs/` | Engineering-Baseline v9 (eingefrorener Referenzpunkt, löst [`archive/MusicBot_ENGINEERING_BASELINE_v8.md`](docs/archive/MusicBot_ENGINEERING_BASELINE_v8.md) ab) + [`MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT, Zwischenstand seit v9-Freeze) + [`FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md) (lebender aktueller Stand) + [`MusicBot_PHASE3_MUSIC_QUALITY_LIBRARY_INTELLIGENCE.md`](docs/MusicBot_PHASE3_MUSIC_QUALITY_LIBRARY_INTELLIGENCE.md) (Phase 3, CLOSED) + Findings-Audits (P0-Metadata/Duplicate-Detection, Artist-Identity-Resolution, Download Pipeline Stability, Metadata Quality, Einzelfunde) + Telegram-Menü-System-Doku + Reprocessing-Tool-Doku + ARCH-Characterization-Dokumente (historisch), siehe [`docs/INDEX.md`](docs/INDEX.md) |
...
`pytest-asyncio` (siehe `requirements-dev.txt`) wird für die `@pytest.mark.asyncio`-Tests in `tests/test_suite.py` benötigt. Aktueller Teststand siehe [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT).
...
Dieses Projekt wird nicht neu geschrieben, sondern kontrolliert weiterentwickelt: bestehendes Verhalten zuerst verstehen und mit Characterization-Tests absichern, dann verbessern. Die verbindlichen Arbeitsregeln stehen in [`CLAUDE.md`](CLAUDE.md). [`docs/MusicBot_ENGINEERING_BASELINE_v9.md`](docs/MusicBot_ENGINEERING_BASELINE_v9.md) ist der eingefrorene technische Referenzpunkt zum Freeze-Zeitpunkt 2026-09-07 (löst [`docs/archive/MusicBot_ENGINEERING_BASELINE_v8.md`](docs/archive/MusicBot_ENGINEERING_BASELINE_v8.md), eingefrorener Stand vom 2026-09-02 mit 1698 passed/0 failed, ab; ältere Baselines bleiben unter [`docs/archive/`](docs/archive/) unverändert bestehen). Änderungen seit dem v9-Freeze (ARCH Status, Recent Major Changes, Testzahlen) werden in [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) (DRAFT, noch nicht eingefroren) mitgeschrieben — der tatsächlich *aktuelle* Stand aller offenen/zurückgestellten Punkte steht dagegen laufend gepflegt in [`docs/FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md). Ältere Architektur-Analysen (ARCH-xxx/POST-ARCH-xxx) liegen vollständig erhalten unter [`docs/archive/`](docs/archive/), siehe [`docs/INDEX.md`](docs/INDEX.md).
