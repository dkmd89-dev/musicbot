...
Historisch organisch gewachsenes Hobbyprojekt — siehe [`CLAUDE.md`](CLAUDE.md) für die Engineering-Leitlinien und [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) für den eingefrorenen technischen Referenzpunkt (Freeze 2026-09-14 — Architektur, Testabdeckung, Security, Technical Debt; löst [`docs/archive/MusicBot_ENGINEERING_BASELINE_v9.md`](docs/archive/MusicBot_ENGINEERING_BASELINE_v9.md) ab). Der laufende Stand aller offenen/zurückgestellten Punkte steht in [`docs/FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md).
...
- Family Hub für explizit konfigurierte Familienmitglieder (keine automatische Zuordnung): Familien-Statistik (Top Songs/Künstler, Statistik pro Person, Musik-Champion, Hörzeiten, Monatsentwicklung — aggregiert aus der bestehenden Play-History, keine zweite Statistik-Engine), ein bot-interner Familien-Chat, sowie eine tägliche Familien-Musik-Challenge mit Punktestand (Details: [`docs/MusicBot_TELEGRAM_MENU_SYSTEM.md`](docs/MusicBot_TELEGRAM_MENU_SYSTEM.md) Abschnitt 6)
- Prüft und repariert die eigene Library selbstständig: read-only Health-Scan + sichere Reparaturen über den Telegram-Menüpunkt „🩺 MusicBot Doctor" (Admin-only), löst nach erfolgreichen Reparaturen automatisch einen Navidrome-Scan aus, zeigt eine Library-Statistik-Übersicht (Tracks/Albums/Artists/Genre-Verteilung) und führt pro Download eine Metadata-Checkliste im Verlauf mit (Details: [`docs/LIBRARY_HEALTH.md`](docs/LIBRARY_HEALTH.md), [`docs/LIBRARY_REPAIR.md`](docs/LIBRARY_REPAIR.md))
...
| `services/library_health/` | Read-only Library-Health-Scanner (Muster wie `services/duplicate/`): Discovery + Per-Datei-Analyse (Metadata/Artwork/Lyrics/Audio/Loudness/Genre/Multi-Artist/Struktur) + Group-Analyse (Album-/Artist-Konsistenz, Duplicate-Klassen) + deterministischer Health-Score. Erzeugt nur einen Report, keine Mutationen (technisch nachgewiesen). Details: [`docs/LIBRARY_HEALTH.md`](docs/LIBRARY_HEALTH.md) |
| `services/library_repair/` | Leitet aus dem Health-Report konkrete Reparaturaktionen ab und führt sie aus: Planner (Issue-Code → Repair-Level, read-only) + 7 Executoren (Tag-Fixes, Renames, Cover, Album-Cover-Vereinheitlichung, MusicBrainz-IDs/ISRC, volle Neuverarbeitung, ReplayGain) mit Per-Datei-Backup außerhalb der Library, Audio-Essenz-Verifikation, Rollback bei Fehler und Append-Only-Journal; Duplicate-Auflösung dockt separat an `scripts/resolve_duplicates.py` an. Automatischer Navidrome-Scan nach `--apply` mit echter Änderung. `doctor_runner.py` orchestriert Health-Scan + SAFE_AUTOMATIC-Apply als Subprozesse für den Telegram-Menüpunkt „MusicBot Doctor". Seit ARCH-032 zusätzlich eine zweite, Command-getriebene „Library-Maintenance-Actions"-Schicht (kein Health-Finding-Bezug): Artist-Casing-Korrektur, Legacy-Genre-Bereinigung, gezieltes Genre-Setzen — `artist.py`/`genre.py` (Domain) + drei neue Executor-Funktionen + geteiltes `run_tracking.py` (Lock/Journal/Run-Index mit dem Finding-Flow), erreichbar über `scripts/library_repair.py --maintenance-action` und den Telegram-Menüpunkt „🧹 Library-Wartung". Alle Executoren gegen die Produktions-Library gelaufen. Details: [`docs/LIBRARY_REPAIR.md`](docs/LIBRARY_REPAIR.md) |
...
| `tests/` | 4250 Tests (pytest), 0 bekannte Fehlschläge, 1 umgebungsbedingt übersprungen, 11 subtests passed (Stand 2026-09-14, v10-Freeze — Details: [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md)) — Characterization-Tests für die Produktionsklassen |
| `docs/` | Engineering-Baseline v10 (eingefrorener Referenzpunkt, Freeze 2026-09-14, löst [`archive/MusicBot_ENGINEERING_BASELINE_v9.md`](docs/archive/MusicBot_ENGINEERING_BASELINE_v9.md) ab) + [`FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md) (lebender aktueller Stand) + [`MusicBot_PHASE3_MUSIC_QUALITY_LIBRARY_INTELLIGENCE.md`](docs/MusicBot_PHASE3_MUSIC_QUALITY_LIBRARY_INTELLIGENCE.md) (Phase 3, CLOSED) + Findings-Audits (P0-Metadata/Duplicate-Detection, Artist-Identity-Resolution, Download Pipeline Stability, Metadata Quality, Einzelfunde) + Telegram-Menü-System-Doku + Reprocessing-Tool-Doku + ARCH-Characterization-Dokumente (historisch), siehe [`docs/INDEX.md`](docs/INDEX.md) |
...
`pytest-asyncio` (siehe `requirements-dev.txt`) wird für die `@pytest.mark.asyncio`-Tests in `tests/test_suite.py` benötigt. Aktueller Teststand siehe [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md).
...
Dieses Projekt wird nicht neu geschrieben, sondern kontrolliert weiterentwickelt: bestehendes Verhalten zuerst verstehen und mit Characterization-Tests absichern, dann verbessern. Die verbindlichen Arbeitsregeln stehen in [`CLAUDE.md`](CLAUDE.md). [`docs/MusicBot_ENGINEERING_BASELINE_v10.md`](docs/MusicBot_ENGINEERING_BASELINE_v10.md) ist der eingefrorene technische Referenzpunkt zum Freeze-Zeitpunkt 2026-09-14 (4250 passed/0 failed — löst [`docs/archive/MusicBot_ENGINEERING_BASELINE_v9.md`](docs/archive/MusicBot_ENGINEERING_BASELINE_v9.md), eingefrorener Stand vom 2026-09-07 mit 2580 passed/0 failed, ab; ältere Baselines bleiben unter [`docs/archive/`](docs/archive/) unverändert bestehen). Der tatsächlich *aktuelle* Stand aller offenen/zurückgestellten Punkte steht laufend gepflegt in [`docs/FINDINGS_INDEX.md`](docs/FINDINGS_INDEX.md). Ältere Architektur-Analysen (ARCH-xxx/POST-ARCH-xxx) liegen vollständig erhalten unter [`docs/archive/`](docs/archive/), siehe [`docs/INDEX.md`](docs/INDEX.md).
