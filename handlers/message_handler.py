...
from logger import (
    log_menu_info as log_info,
    log_menu_error as log_error,
    log_menu_warning as log_warning,
    log_menu_debug as log_debug,
)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not (url := extract_youtube_url(message_text)):
            log_warning(
                "Invalid input received - not a YouTube URL", context="message_handler"
            )
...
async def process_audio_download(url: str, update: Update, user_id: int) -> None:
    ...
    status_msg = await update.message.reply_text(
        "⏳ *Download wird gestartet...*", parse_mode="Markdown"
    )
    ...
    try:
        ...
        if processed_result:
            log_info(
                f"Download erfolgreich: {processed_result.get('file_path')}",
                context="message_handler",
            )
            await handle_download_success(update, status_msg, processed_result)
        else:
            await handle_download_failure(
                update, status_msg, "Download returned no result."
            )
    except Exception as e:
        error_message = f"❌ Unerwarteter Fehler: {str(e)}"
        log_error(error_message, context="message_handler", exc_info=True)
    ...
    if isinstance(result, dict):
        if result.get("success"):
            ...
            response["error"] = result.get(
                "error", "Download fehlgeschlagen (unbekannter Fehler)"
            )
...
async def handle_download_success(
    update: Update, status_msg: Message, result: Union[str, List[str]]
) -> None:
    """Send success message after download"""
    try:
        await status_msg.edit_text(
            f"✅ *Download abgeschlossen!*\n\n"
            f"Datei: `{result.get('filename', 'unbekannt')}`",
            parse_mode="Markdown",
        )
    except Exception as e:
        log_error(
            f"Fehler beim Senden der Erfolgsnachricht: {str(e)}",
            context="message_handler",
        )


async def handle_download_failure(
    update: Update, status_msg: Message, error_msg: str
) -> None:
    """Send failure message after download"""
    log_error(f"Download fehlgeschlagen: {error_msg}", context="message_handler")
    ...
    try:
        ...
        log_error(
            f"Fehler beim Senden der Download-Fehlermeldung: {str(e)}",
            context="message_handler",
        )
